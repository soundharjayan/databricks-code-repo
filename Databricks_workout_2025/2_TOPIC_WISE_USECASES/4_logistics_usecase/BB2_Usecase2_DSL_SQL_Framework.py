...
# MAGIC Cleansing:
# MAGIC Null handling
# MAGIC Defaults for missing values
# MAGIC
# MAGIC Enrichment:
# MAGIC Load timestamp
# MAGIC Shipment year derivation
# MAGIC
# MAGIC Integration:
# MAGIC Union multiple sources
# MAGIC Join staff + shipment
# MAGIC
# MAGIC Analytics Ready Output:
# MAGIC Clean, enriched DataFrame

# COMMAND ----------

# MAGIC %md
# MAGIC Common Functions/Utilities creation
...
def read_json_df(spark, path,mline=True):
    return spark.read.json(path,multiLine=mline)
...
sourcedf1=read_csv_df(spark,'/Volumes/workspace/default/volume1/logistics_source1',True,False)
...
sourcedffinal.where("source_system='system2'").show(2)

# COMMAND ----------

jsondf=read_json_df(spark,'/Volumes/workspace/default/volume1/logistics_shipment_detail_3000.json',True)
jsondf.show()
...
def clean_and_scrub_generic(df, drop_configs, replace_configs, fill_defaults, type_casts):
...
drop_rules = [(["shipment_id", "role"], "any"),(["first_name", "last_name"], "all")]
...
jsondf=process_shipment_details(jsondf)
jsondf.show(2)

# COMMAND ----------

def finalize_staff_data(df):
    """
...
mergeddf=finalize_staff_data(cleanse_scrub_final_df)
mergeddf.show(2)

# COMMAND ----------

def apply_deduplication(df, primary_key_cols=None):
    """
...
mergeddf=apply_deduplication(mergeddf)
mergeddf.show(2)
...
df_shipments_processed = process_shipment_details(df_json)
