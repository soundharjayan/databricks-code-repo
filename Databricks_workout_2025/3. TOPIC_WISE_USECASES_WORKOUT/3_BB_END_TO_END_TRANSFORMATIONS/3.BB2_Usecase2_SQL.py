...
df = spark.read.csv("/Volumes/workspace/default/logistics_vol/logistics_data/logistics_source1",header=True,inferSchema = True)
display(df)
...
# MAGIC %sql
# MAGIC -- 1.Apply inferSchema and toDF to create a DF and analyse the actual data.
# MAGIC CREATE OR REPLACE TEMPORARY VIEW df_view 
# MAGIC USING CSV
# MAGIC OPTIONS (
# MAGIC   PATH "/Volumes/workspace/default/logistics_vol/logistics_data/logistics_source1",
# MAGIC   header "True",
...
# MAGIC -- 2.Analyse the schema, datatypes, columns etc.,
# MAGIC
# MAGIC -- read_df.printSchema()
# MAGIC DESCRIBE df_view;
# MAGIC
# MAGIC -- display(read_df.columns)
# MAGIC SHOW COLUMNS IN df_view;
# MAGIC
# MAGIC -- display(read_df.dtypes)
# MAGIC -- display(read_df.schema)
# MAGIC -- There is no schema and dtypes concept in Sql
# MAGIC
# MAGIC -- 3.Analyse the duplicate records count and summary of the dataframe.
# MAGIC select count(*) as total_count from df_view;
# MAGIC
# MAGIC select count(*) as distinct_all_columns_count  
# MAGIC from (
# MAGIC   select distinct * from df_view
# MAGIC )t;
# MAGIC
# MAGIC select count(*) as distinct_of_id_columns_count
# MAGIC from (select distinct shipment_id 
# MAGIC from df_view)t;
# MAGIC
...
df = spark.read.csv(path =["/Volumes/workspace/default/logistics_vol/logistics_data/logistics_source1","/Volumes/workspace/default/logistics_vol/logistics_data/logistics_source2"],header=True,inferSchema = True)
display(df)
df.createOrReplaceTempView("passive_munging_view")
...
# MAGIC
# MAGIC -- 1. shipment_id is non-numeric
# MAGIC -- 2. age is not an integer
# MAGIC
# MAGIC select shipment_id
# MAGIC from passive_munging_view
# MAGIC where shipment_id not rlike '^[0-9]+$';
# MAGIC
# MAGIC select age
# MAGIC from passive_munging_view
# MAGIC where age not rlike '^[0-9]+$';
# MAGIC
# MAGIC -- 3. fewer columns than expected 
# MAGIC -- 4. more columns than expected
# MAGIC
# MAGIC -- Sql is difficult to achieve this. Easier way,We can achieve by using DSL in the mode of permissive and corruptedrows.
# MAGIC
...
# MAGIC %sql
# MAGIC -- 1.Read both files without enforcing schema
# MAGIC CREATE OR REPLACE TEMPORARY VIEW df1_view 
# MAGIC USING CSV
# MAGIC OPTIONS (
# MAGIC   PATH "/Volumes/workspace/default/logistics_vol/logistics_data/logistics_source1",
# MAGIC   header "True"
# MAGIC );
# MAGIC
# MAGIC CREATE OR REPLACE TEMPORARY VIEW df2_view 
# MAGIC USING CSV
# MAGIC OPTIONS (
# MAGIC   PATH "/Volumes/workspace/default/logistics_vol/logistics_data/logistics_source2",
# MAGIC   header "True"
# MAGIC );
# MAGIC
# MAGIC -- 2.Add data_source column with values as: system1, system2 in the respective dataframes
# MAGIC CREATE OR REPLACE TEMPORARY VIEW df1_enriched_view AS
# MAGIC SELECT *, 'system1' AS data_source
# MAGIC FROM df1_view;
# MAGIC
# MAGIC CREATE OR REPLACE TEMPORARY VIEW df2_enriched_view AS
# MAGIC SELECT *, 'system2' AS data_source
# MAGIC FROM df2_view;
# MAGIC
# MAGIC -- 3.Align them into a single canonical schema: shipment_id, first_name, last_name, age, role, hub_location, vehicle_type, data_source
# MAGIC -- The ANSI SQL doesn't support schema merging (unionByName).But it support union and unionall if the both table/view columns are matched,otherwise it won't support
# MAGIC
# MAGIC
