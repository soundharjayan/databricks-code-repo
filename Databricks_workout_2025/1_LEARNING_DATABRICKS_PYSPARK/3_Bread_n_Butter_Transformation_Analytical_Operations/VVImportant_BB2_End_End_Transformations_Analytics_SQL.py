...
# MAGIC Only the usage of SQL/Declarative Language Programming
# MAGIC ###Points to consider using SQL rather than DSL
# MAGIC 1. We can't use pure sql alone for achieving all functionalities.
# MAGIC 2. We will use SQL/DSL appropriately as and when it is applicable.
# MAGIC 3. We use DSL in data ingestion/egress mostly, using SQL in ingestion/egress is bit challenging.
# MAGIC 4. Performance wise usage of SQL or DSL is going to almost same, because any way SQL/DSL internally spark is optimizing the code and convert the code in RDD.
# MAGIC 5. Anytime we can represent DF to SQL or SQL to DF easily.
...
# MAGIC %sql
# MAGIC --pure sql without any FBP/DSL
# MAGIC SELECT * from rawdf1view limit 10;

# COMMAND ----------

# MAGIC %sql
# MAGIC --For extraction/load operations with multiple features, preferably use DSL and not SQL
# MAGIC --For initial dataset creation or for ingestion/egress DSL way of creating dataframe is better, because not all the options are supported in SQL ingestion
...
# MAGIC   age INT,
# MAGIC   profession STRING
# MAGIC )
# MAGIC USING CSV
# MAGIC OPTIONS (
# MAGIC   path "/Volumes/workspace/default/volume1/custs",
# MAGIC   header "false",
# MAGIC   inferSchema "false" 
# MAGIC );
# MAGIC
# MAGIC select * from rawdf1view limit 20;

# COMMAND ----------

# MAGIC %md
# MAGIC For creating initial dataframe, SQL is not better means, if I wanted to write SQL query after dataframe creation? We have to represent Dataframe to table/view format, so we can write SQL query..
...
# MAGIC %sql
# MAGIC --We can directly create tempview from the source
# MAGIC CREATE OR REPLACE TEMPORARY VIEW rawdf1view
# MAGIC (
# MAGIC   id INT,
# MAGIC   firstname STRING,
...
# MAGIC %sql
# MAGIC select * from rawdf1view WHERE RAND() <= 0.1;
...
# MAGIC %sql
# MAGIC --inline view (from clause subquery)
# MAGIC SELECT COUNT(*) AS dedup_all_columns_count
# MAGIC FROM (SELECT DISTINCT * FROM rawdf1view) t;

# COMMAND ----------

# MAGIC %sql
# MAGIC --Importantly, we are using qualify function (advanced feature) to avoid additional temporary views (refer next cell)
# MAGIC --df.dropDuplicates(['id']).count()
# MAGIC SELECT COUNT(*) AS dedup_id_count
# MAGIC FROM (
# MAGIC   SELECT *
# MAGIC   FROM rawdf1view
# MAGIC   qualify ROW_NUMBER() OVER (PARTITION BY id ORDER BY id)=1) inlineview;
...
# MAGIC FROM 
# MAGIC (
# MAGIC   select id,age from 
# MAGIC (
# MAGIC   SELECT *,ROW_NUMBER() OVER (PARTITION BY id ORDER BY id) rno
# MAGIC   FROM rawdf1view) inlineview
# MAGIC   where rno = 1) inlineview2;
# MAGIC   
...
# MAGIC %md
# MAGIC **Above multi files handling and schema merging is best handled by DSL rather than SQL.**
...
# MAGIC %sql
# MAGIC CREATE OR REPLACE TEMP VIEW cleandf1 (
# MAGIC   id INT,
# MAGIC   firstname STRING,
# MAGIC   lastname STRING,
# MAGIC   age SMALLINT,
# MAGIC   profession STRING,
# MAGIC   corruptedrows STRING
# MAGIC )
# MAGIC USING CSV
# MAGIC OPTIONS (
# MAGIC   path "/Volumes/workspace/wd36schema/ingestion_volume/source/custsmodified",
# MAGIC   mode "PERMISSIVE",
# MAGIC   columnNameOfCorruptRecord "corruptedrows",
# MAGIC   header "false"
# MAGIC );
...
# MAGIC %sql
# MAGIC --DSL is better to use, we don't have to manage all the columns in the filter with not() or and operators
# MAGIC --rawdf1.na.drop(how="any",subset=["id","age"])
# MAGIC CREATE OR REPLACE TEMP VIEW cleanseddf_any AS
# MAGIC SELECT *
# MAGIC FROM cleandf1
# MAGIC WHERE id IS NOT NULL AND age IS NOT NULL;
# MAGIC
# MAGIC --rawdf1.na.drop(how="all",subset=["firstname","lastname"])
# MAGIC CREATE OR REPLACE TEMP VIEW cleanseddf_all AS
# MAGIC SELECT *
# MAGIC FROM cleandf1
# MAGIC WHERE NOT (firstname IS NULL AND lastname IS NULL);
# MAGIC
# MAGIC
...
# MAGIC %sql
# MAGIC --na.fill equivalent cleanseddf.na.fill('not provided',subset=["lastname","profession"])
# MAGIC CREATE OR REPLACE TEMP VIEW scrubbeddf1 AS
# MAGIC SELECT
# MAGIC   id,
# MAGIC   firstname,
# MAGIC   COALESCE(lastname, 'not provided')  AS lastname,
# MAGIC   age,
# MAGIC   COALESCE(profession, 'not provided') AS profession
# MAGIC FROM cleandf1;
# MAGIC
# MAGIC --na.replace equivalent scrubbeddf1.na.replace({'Pilot':'Captain','Actor':'Celeberity'})
# MAGIC CREATE OR REPLACE TEMP VIEW scrubbeddf2 AS
# MAGIC SELECT
# MAGIC   id,
# MAGIC   firstname,
# MAGIC   lastname,
# MAGIC   age,
# MAGIC   CASE
# MAGIC     WHEN profession = 'Pilot' THEN 'Captain'
# MAGIC     WHEN profession = 'Actor' THEN 'Celeberity'
# MAGIC     ELSE profession
# MAGIC   END AS profession
# MAGIC FROM scrubbeddf1;
# MAGIC
...
# MAGIC %sql
# MAGIC --dedupdf2=dedupdf1.coalesce(1).orderBy(["id","age"],ascending=[True,False]).dropDuplicates(subset=["id"])
# MAGIC --better to use qualify
# MAGIC CREATE OR REPLACE TEMP VIEW dedupdf2_priority AS
# MAGIC SELECT *
# MAGIC FROM (
# MAGIC   SELECT *,
# MAGIC          ROW_NUMBER() OVER (
# MAGIC            PARTITION BY id
# MAGIC            ORDER BY id,age DESC
# MAGIC          ) AS rn
# MAGIC   FROM scrubbeddf2
# MAGIC ) t
# MAGIC WHERE rn = 1;
# MAGIC select * from dedupdf2_priority;

# COMMAND ----------

# MAGIC %sql
# MAGIC --show me the duplicate data alone
# MAGIC CREATE OR REPLACE TEMP VIEW duplicateview AS
# MAGIC SELECT *
# MAGIC FROM (
# MAGIC   SELECT *,
# MAGIC          ROW_NUMBER() OVER (PARTITION BY id ORDER BY id,age DESC) AS rn
# MAGIC   FROM scrubbeddf2
# MAGIC ) t;
# MAGIC --select * from duplicateview where rn=1;--deduplicated data
# MAGIC select * from duplicateview where rn>1;--duplicate data alone
...
# MAGIC %sql
# MAGIC --DSL is better and line of code is less
# MAGIC CREATE OR REPLACE TEMP VIEW standarddf1 AS
# MAGIC SELECT
# MAGIC   *,
# MAGIC   'Retail' AS sourcesystem
# MAGIC FROM dedupdf2_priority 
# MAGIC where upper(id) =lower(id);--wanted to take only number values in id column
# MAGIC
# MAGIC select * from standarddf1 limit 10;
# MAGIC
...
# MAGIC %sql
# MAGIC CREATE OR REPLACE TEMP VIEW standarddf2 AS
# MAGIC SELECT
# MAGIC   id,
# MAGIC   firstname,
# MAGIC   lastname,
# MAGIC   age,
# MAGIC   INITCAP(profession) AS profession,
# MAGIC   sourcesystem
# MAGIC FROM standarddf1;
# MAGIC
# MAGIC select * from standarddf2 limit 10;
...
# MAGIC %sql
# MAGIC CREATE OR REPLACE TEMP VIEW standarddf3 AS
# MAGIC SELECT
# MAGIC   CASE
# MAGIC     WHEN cast(id as string) = 'nine'  THEN '9'
# MAGIC     WHEN cast(id as string) = 'ten'   THEN '10'
# MAGIC     ELSE id
# MAGIC   END AS id,
# MAGIC   firstname,
# MAGIC   lastname,
# MAGIC   REGEXP_REPLACE(age, '-', '') AS age,
# MAGIC   profession,
# MAGIC   sourcesystem
# MAGIC FROM standarddf2;
# MAGIC
# MAGIC select * from standarddf3 limit 10;
# MAGIC
...
# MAGIC %sql
# MAGIC create or replace temp view standarddf4 as 
# MAGIC SELECT 
# MAGIC     CAST(id AS BIGINT) AS id,
# MAGIC     firstname,
# MAGIC   lastname,
# MAGIC   CAST(age AS SMALLINT) AS age,
# MAGIC   profession,
# MAGIC   sourcesystem
# MAGIC FROM standarddf3;
...
# MAGIC %sql
# MAGIC create or replace temp view standarddf5 as 
# MAGIC SELECT 
# MAGIC     id custid,
# MAGIC     firstname,
# MAGIC     lastname,
# MAGIC     age,
# MAGIC     profession,
# MAGIC     sourcesystem AS srcsystem
# MAGIC FROM standarddf4;
...
# MAGIC %sql
# MAGIC create or replace temp view mungeddf as 
# MAGIC SELECT 
# MAGIC     custid, 
# MAGIC     age, 
# MAGIC     firstname,
...
# MAGIC     profession,
# MAGIC     srcsystem
# MAGIC FROM standarddf5;
# MAGIC
# MAGIC SELECT * FROM mungeddf LIMIT 10;
...
# MAGIC %sql
# MAGIC select * from enrichdf1 limit 10;
...
# MAGIC %sql
# MAGIC --Rather than using DSL, a single Select can achieve add/remove/rename/replace/modify the given columns, so sql is better to use for performing all in one select sql
# MAGIC create or replace temp view enrichdf2 as 
# MAGIC SELECT 
# MAGIC     *,
# MAGIC     substring(profession, 1, 1) AS professionflag
# MAGIC FROM enrichdf1;
# MAGIC
# MAGIC SELECT * FROM enrichdf2 LIMIT 20;
...
# MAGIC %sql
# MAGIC create or replace temp view enrichdf3 as 
# MAGIC SELECT 
# MAGIC     custid,
# MAGIC     age,
# MAGIC     firstname,
# MAGIC     lastname,
# MAGIC     profession,
# MAGIC     srcsystem AS sourcename, -- Renaming occurs here
# MAGIC     datadt,
# MAGIC     loaddt,
# MAGIC     professionflag AS profflag -- Renaming occurs here
# MAGIC FROM enrichdf2;
# MAGIC
# MAGIC SELECT * FROM enrichdf3 LIMIT 20;
...
# MAGIC %sql
# MAGIC create or replace temp view enrichdf4 as 
# MAGIC SELECT 
# MAGIC     custid,
# MAGIC     age,
# MAGIC     firstname,
# MAGIC     lastname,
# MAGIC     concat(profession, '-', profflag) AS profession, -- Overwrites/Modifies the profession column
# MAGIC     sourcename,
# MAGIC     datadt,
# MAGIC     loaddt,
# MAGIC     profflag
# MAGIC FROM enrichdf3;
# MAGIC
# MAGIC SELECT * FROM enrichdf4 LIMIT 20;
...
# MAGIC %sql
# MAGIC create or replace temp view enrichdf5 as 
# MAGIC SELECT 
# MAGIC     custid,
# MAGIC     age,
# MAGIC     firstname,
...
# MAGIC     datadt,
# MAGIC     loaddt
# MAGIC FROM enrichdf4;
# MAGIC
# MAGIC SELECT * FROM enrichdf5 LIMIT 20;
...
# MAGIC %sql
# MAGIC create or replace temp view mergeddf as 
# MAGIC SELECT 
# MAGIC     custid,
# MAGIC     age,
# MAGIC     concat_ws(' ', firstname, lastname) AS fullname, -- Merging firstname and lastname
# MAGIC     split(profession, '-')[0] AS profession, -- Splitting and taking the first element
# MAGIC     sourcename,
# MAGIC     datadt,
# MAGIC     loaddt,
# MAGIC     upper(substring(split(profession, '-')[0], 1, 3)) AS shortprof -- creating shortprof based on the split result
# MAGIC FROM enrichdf5;
# MAGIC
# MAGIC SELECT * FROM mergeddf LIMIT 10;
...
# MAGIC %sql
# MAGIC --All the above enrichments can be writtern in a single SQL select.
# MAGIC create or replace temp view formatteddf as 
# MAGIC SELECT 
# MAGIC     custid,
# MAGIC     age,
# MAGIC     fullname,
# MAGIC     profession,
# MAGIC     sourcename,
# MAGIC     to_date(datadt, 'yy/dd/MM') AS datadt, -- Converts string '25/30/12' to Date '2025-12-30'
# MAGIC     loaddt,
# MAGIC     shortprof
# MAGIC FROM mergeddf;
# MAGIC SELECT * FROM formatteddf LIMIT 10;
...
# MAGIC %sql
# MAGIC create or replace temp view customdf as 
# MAGIC SELECT 
# MAGIC     *,
# MAGIC     sql_age_cat(age) AS agecat
# MAGIC FROM formatteddf;
# MAGIC
# MAGIC SELECT * FROM customdf LIMIT 10;
...
# MAGIC %sql
# MAGIC --sql is familiar to use rather than using dsl
# MAGIC create or replace temp view filterdf as 
# MAGIC select * from customdf
# MAGIC where age>40 and age<=60;
# MAGIC select * from filterdf;
...
# MAGIC %sql
# MAGIC --SQL is better to use than DSL because SQL familiar and simplified syntax
# MAGIC --One drawback of DSL is, DSL is platform agnostic (work only in Spark DF) but SQL is non-platform agnostic (works anywhere)
# MAGIC create or replace temp view curateddf as 
# MAGIC select *,case when age is null then 'Unknown' 
# MAGIC                               when age<=10 then 'child' 
# MAGIC                               when age>10 and age<=19 then 'teenager' 
# MAGIC                               when age>19 and age<=30 then 'young'
# MAGIC                               when age>30 and age<=50 then 'middleaged'
# MAGIC                               else 'oldaged' end as agecat1 
# MAGIC                               from customdf ;
# MAGIC select * from curateddf;
...
# MAGIC %sql
# MAGIC create or replace temp view curateddf3 as 
# MAGIC select *,
# MAGIC datediff(loaddt,datadt) as delaydays,year(datadt) as datayear
# MAGIC                             ,month(datadt) as datamonth
# MAGIC                             ,last_day(datadt)as datalastday,initcap(agecat) as agecat2 from curateddf;
# MAGIC select * from curateddf3;
...
# MAGIC %sql
# MAGIC --SQL is more familiar and easy for lift and shift operation
# MAGIC create or replace temp view curateddf4 as 
# MAGIC select profession,agecat,count(custid) as custcount,avg(age) as avgage
# MAGIC from curateddf3
# MAGIC group by profession,agecat;
# MAGIC select * from curateddf4 limit 10;
...
# MAGIC %sql
# MAGIC create or replace temp view curateddf5 as 
# MAGIC select *
# MAGIC from curateddf4
# MAGIC order by profession,agecat;
# MAGIC select * from curateddf5 limit 10;
...
# MAGIC %sql
# MAGIC --Wanted to do lookup, whether customer has transacted in last month or not?
# MAGIC create or replace temp view custonlylastmonthtransacteddf as 
# MAGIC SELECT cust.* 
# MAGIC --cust.age,cust.fullname,cust.profession,cust.sourcename,cust.datadt, cust.loaddt,cust.shortprof,cust.agecatflag,cust.agecat,t.*
# MAGIC FROM custdimdftv cust
# MAGIC LEFT SEMI JOIN txnsdecdftv t ON cust.custid = t.custid;
# MAGIC select * from custonlylastmonthtransacteddf limit 10;
...
# MAGIC %sql
# MAGIC --Wanted to do lookup & enrichment, transactions details are enriched for the given customer
# MAGIC create or replace temp view custonlylastmonthtransacteddf as 
# MAGIC SELECT cust.age,cust.fullname,cust.profession,cust.sourcename,cust.datadt, cust.loaddt,cust.shortprof,cust.agecatflag,cust.agecat,t.*
# MAGIC FROM custdimdftv cust
# MAGIC LEFT JOIN txnsdecdftv t ON cust.custid = t.custid;
# MAGIC select * from custonlylastmonthtransacteddf limit 10;
...
# MAGIC 1. Creating Surrogate/primary key/seq number
...
# MAGIC %sql
# MAGIC --Comparing with DSL, SQL is more familiar and simple in syntax for writing and reasoning.
# MAGIC --orderjoineddf.withColumn("rnk",row_number().over(Window.partitionBy("custid").orderBy(desc("spendby")))).withColumn("densernk",dense_rank().over(Window.partitionBy("custid").orderBy(desc("spendby"))))
# MAGIC --no need of extending pyspark.sql.window import Window, no need of desc(), no need of using camelCase functions etc.,
# MAGIC select *,
# MAGIC row_number() over(partition by custid order by txndt desc) as rno,
# MAGIC rank() over(partition by custid order by txndt desc) as rnk,
# MAGIC dense_rank() over(partition by custid order by txndt desc) as drnk
# MAGIC from orderjoineddftv 
# MAGIC limit 100;
...
# MAGIC Performing analytics/summarization/categorization of data by applying/not by applying windowing functionality.
...
# MAGIC %sql
# MAGIC create or replace temp view aggranalytical_tv as
# MAGIC SELECT 
# MAGIC     *,
# MAGIC     CASE 
# MAGIC         WHEN priortransamt = -1 THEN 'first transaction'
# MAGIC         WHEN amt > priortransamt THEN 'increase'
# MAGIC         WHEN amt < priortransamt THEN 'decrease'
# MAGIC         ELSE 'same'
# MAGIC     END AS transpattern
# MAGIC FROM
# MAGIC     (SELECT 
# MAGIC         *,
# MAGIC         LAG(amt, 1, -1) OVER (PARTITION BY custid ORDER BY txndt ASC) AS priortransamt,
# MAGIC         LEAD(amt, 1, -1) OVER (PARTITION BY custid ORDER BY txndt ASC) AS nexttransamt
# MAGIC     FROM orderjoineddftv ) lag_lead_calculated;
# MAGIC
# MAGIC select * from aggranalytical_tv limit 100;
...
# MAGIC SELECT 
# MAGIC     *,
# MAGIC     FIRST_VALUE(amt) OVER (
# MAGIC         PARTITION BY custid 
# MAGIC         ORDER BY txndt ASC
# MAGIC     ) AS firsttransvalue,
# MAGIC     FIRST_VALUE(amt) OVER (
# MAGIC         PARTITION BY custid 
# MAGIC         ORDER BY txndt DESC
# MAGIC     ) AS lasttransvalue,
# MAGIC     CUME_DIST() OVER (
# MAGIC         PARTITION BY custid 
# MAGIC         ORDER BY txndt DESC
# MAGIC     ) AS cumedistribution
# MAGIC FROM orderjoineddftv;
...
# MAGIC %sql
# MAGIC --sql is more familiar and english like language
# MAGIC SELECT 
# MAGIC     product,
# MAGIC     spendby,
# MAGIC     SUM(amt) AS sumamt,
# MAGIC     AVG(amt) AS avgamt
# MAGIC FROM orderjoineddftv
# MAGIC where product<>'Exercise & Fitness'
# MAGIC GROUP BY product, spendby
# MAGIC having SUM(amt)>100
# MAGIC ORDER BY 
# MAGIC     product ASC,
# MAGIC     sumamt DESC;
...
# MAGIC     AVG(amt) AS avgamt
# MAGIC FROM orderjoineddftv
# MAGIC GROUP BY ROLLUP (state, city)
# MAGIC ORDER BY state, sumamt;
...
# MAGIC     AVG(amt) AS avgamt
# MAGIC FROM orderjoineddftv
# MAGIC GROUP BY CUBE (state, category)
# MAGIC ORDER BY state, sumamt;

# COMMAND ----------

# MAGIC %sql
# MAGIC --Pivot in SQL is not dynamic/not simple as like DSL, hence DSL is better option for doing Pivot
# MAGIC SELECT * FROM (
# MAGIC   SELECT 
# MAGIC     state, 
# MAGIC     city, 
...
# MAGIC     amt 
# MAGIC   FROM orderjoineddftv
# MAGIC ) as originaldata
# MAGIC PIVOT (
# MAGIC   SUM(amt) AS sum_amt, 
# MAGIC   AVG(amt) AS avg_amt
# MAGIC   FOR category IN ('Exercise & Fitness', 'Jumping')--Additional effort in SQL
# MAGIC )
...
# MAGIC     state,
# MAGIC     city,
# MAGIC     SUM(CASE WHEN category = 'Exercise & Fitness' THEN amt ELSE 0 END) as exercise_fitness_Sum,
# MAGIC     SUM(CASE WHEN category = 'Jumping' THEN amt ELSE 0 END) as Jumping_Sum
# MAGIC FROM orderjoineddftv
# MAGIC GROUP BY state, city
# MAGIC ORDER BY state;
...
# DBTITLE 1,Subtract df2 from df1
# MAGIC %sql
# MAGIC CREATE OR REPLACE TEMP VIEW df1 AS
# MAGIC SELECT *
# MAGIC FROM cleandf1
# MAGIC WHERE id BETWEEN 4000100 AND 4000110;
# MAGIC
# MAGIC CREATE OR REPLACE TEMP VIEW df2 AS
# MAGIC SELECT *
# MAGIC FROM cleandf1
# MAGIC WHERE id BETWEEN 4000105 AND 4000115;
# MAGIC
# MAGIC --set operations
# MAGIC --Thumb rules to perform set operations:
# MAGIC --1. Column numbers should be same
# MAGIC --2. Column order should be same
# MAGIC --3. Column datatype should be same
# MAGIC

# COMMAND ----------

# MAGIC %sql
# MAGIC --union (returns union of multiple dfs with duplicates)
...
# MAGIC
# MAGIC %sql
# MAGIC --unionall (alias of union/same as union)
# MAGIC SELECT * FROM df1
# MAGIC UNION ALL
# MAGIC SELECT * FROM df2;
# MAGIC
...
# MAGIC %sql
# MAGIC --unionByName (we don't have available in SQL, we need to handle explicitly, hence DSL is better for schema merging)
# MAGIC SELECT
# MAGIC     id,age, profession,firstname,lastname,'retailsource' sourcesystem
# MAGIC FROM df1
# MAGIC UNION ALL
# MAGIC SELECT id,age,profession,firstname,lastname,'retailsource' AS sourcesystem
# MAGIC FROM df2;
# MAGIC
# MAGIC
# MAGIC
...
# MAGIC %sql
# MAGIC --intersect (returns common data between both df excluding duplicates)
# MAGIC SELECT * FROM df1
# MAGIC INTERSECT
# MAGIC SELECT * FROM df2;
# MAGIC
...
# MAGIC %sql
# MAGIC --subtract (returns df1 - df2)
# MAGIC SELECT * FROM df1
# MAGIC minus
# MAGIC SELECT * FROM df2;
